...
export class PlayerEntity {
  ...
  enableMovement() {
    ...
    k.onKeyPress((_key: Key) => {
      if(player.isAttacking || player.isShielding || player.isInteracting || globalState.freezePlayer) return;
      if(_key === "space") {
  ...
  public async handleSwordAttack(key: Key) {
    ...
    if (k.get("swordHitBox").length === 0) {
      const swordHitBoxDimensions = this.getSwordHitBoxDimensions(player.direction);
      ...
      const swordHitBoxGameObj = k.add([
        ...
        k.area({
          shape: new k.Rect(k.vec2(0), swordHitBoxDimensions[0], swordHitBoxDimensions[1]),
        }),
